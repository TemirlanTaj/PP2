...
class Enemy(pygame.sprite.Sprite):
      ...
      def move(self):
        ...
        if (self.rect.bottom > 600):
            # moveSpeed += 0.75 now we change speed when player earns coins
            # score += 1 now we get points from coins
            self.rect.top = 0
            self.rect.center = (random.randint(40, Width - 40), 0)
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(self.image, (20, 20)) # recsale coin because it was too big
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(20, Width - 20), 0)
        #adding random weight
        self.weight = random.randint(1,3)
        self.image.blit(font_small.render(f"{self.weight}", True, colorBlack), (4, -3))
...
while running:
    ...
    #move sprites and draw them again
    for entity in all_sprites:
        entity.move()
        screen.blit(entity.image, entity.rect)
        
    #check collision of player with coin
    if pygame.sprite.spritecollideany(player, coins): 
        coin.rect.top = 600
        coin.rect.center = (random.randint(20, Width - 20), 0)
        score += coin.weight
        coin.weight = random.randint(1,3)
        coin.image.blit(font_small.render(f"{coin.weight}", True, colorBlack), (4, -3))
