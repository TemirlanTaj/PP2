...
# db.execute_query(db.query_create_table_users)
# db.execute_query(db.query_create_table_user_scores)
...
food = Food()
db.input_user()
best_lvl = db.show_highest_level()

if best_lvl and best_lvl[0][0]: # Check if the result is not None (best_lvl[0][0] because [(5,)])
    print(f"Best level for {db.current_user}: {best_lvl[0][0]}")
else:
    print(f"No scores found for {db.current_user}. Welcome!")
